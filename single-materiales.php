...
<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
        ...
        <main class="bg-[#F5F3EE] pt-32 pb-24 min-h-screen">
            <div class="max-w-7xl mx-auto px-6">
                ...
                <a href="<?php echo get_post_type_archive_link('materiales'); ?>" class="inline-flex items-center gap-2 text-[#555770] hover:text-[var(--color-primary)] text-sm font-500 transition-colors mb-8">
                    <span>←</span> Volver a Materiales
                </a>

                <header class="bg-white p-8 md:p-12 border border-stone-200 mb-8">
                    <?php
                    $terms = get_the_terms(get_the_ID(), 'categoria_material');
                    if ($terms && !is_wp_error($terms)) { ?>
                        <div class="text-[var(--color-primary)] text-xs font-600 tracking-widest uppercase mb-4">
                            <?php echo esc_html($terms[0]->name); ?>
                        </div>
                    <?php } ?>

                    <h1 class="text-3xl md:text-4xl font-800 text-[var(--color-base)] leading-tight mb-6">
                        <?php the_title(); ?>
                    </h1>

                    <?php if (has_excerpt()) : ?>
                        <p class="text-lg text-[#3D3F52] font-400 leading-relaxed border-l-4 border-[var(--color-primary)] pl-4">
